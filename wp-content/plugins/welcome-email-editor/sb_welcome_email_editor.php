...
function sb_we_loaded() {
	...
	if ( is_admin() && ! empty( $_REQUEST['_wp_http_referer'] ) ) {
		if ( ! $sb_we_active ) {
			add_action( 'admin_notices', 'sb_we_admin_notice' );
		}
...
function sb_we_get_settings() {
	...
	return $sb_we_settings;
}

function sb_we_text_html() {
	return "text/html";
}
...
function sb_we_set_email_filter_headers( $reset = false ) {
	if ( $reset ) {
		...
		remove_filter( 'wp_mail_content_type', 'sb_we_text_html', 1, 1 );
		...
		if ( $send_as = $sb_we_settings->header_send_as ) {
			if ( $send_as == 'html' ) {
				add_filter( 'wp_mail_content_type', 'sb_we_text_html', 1, 1 );
...
if ( ! function_exists( 'wp_new_user_notification' ) ) {
	...
	function wp_new_user_notification( $user_id, $depracated = '', $notify = '' ) {
		...
		$sb_we_settings = sb_we_get_settings();
		...
		if ( $user = new WP_User( $user_id ) ) {
			...
			if ( $send_as = $settings->header_send_as ) {
				if ( $send_as == 'html' ) {
					...
					add_filter( 'wp_mail_content_type', 'sb_we_text_html', 1, 100 );
